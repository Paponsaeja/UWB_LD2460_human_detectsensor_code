...
# -------------------------------------------------------------
def start_dns_redirect(ap_ip="192.168.4.3"):
    def dns_thread():
...
def start_config_portal():
    ...
    ap.active(True)
    ap.config(essid="UWB_NODE05", password="12345678")
    ip = ap.ifconfig()[0]
...
def connect_mqtt(config):
    try:
        ...
        # ตั้งค่า Last Will (ประกาศเมื่อ Node หลุด)
        will_topic = f"uwb/{config['mqtt_client_id']}/status"
        client.set_last_will(will_topic, "offline", retain=True)
...
# Callback (เฉพาะคำสั่งของ node นี้เท่านั้น)
# -------------------------------------------------------------
def on_message(topic, msg):
    global client, mqtt_topic, current_count
...
if __name__ == "__main__":
    ...
    client_id = config["mqtt_client_id"]
    mqtt_topic = f"uwb/{client_id}/count"  # send
    cmd_topic = f"uwb/{client_id}/cmd"     # recieve
    ping_topic = f"uwb/{client_id}/ping"   # ping
    # Subscribe เฉพาะคำสั่งของตัวเอง
    ...
    while True:
        try:
            ...
            now = time.ticks_ms()
            if time.ticks_diff(now, last_ping_time) > PING_INTERVAL:
                try:
                    client.publish(ping_topic, b'1', qos=0)
                    last_ping_time = now
                except:
                    pass # ถ้า ping ไม่ผ่าน เดี๋ยว check_msg ข้างล่างจะจัดการเอง
            ...
            time.sleep(2)
            # ใส่ Logic Soft Reconnect ตรงนี้ (ตามโค้ดก่อนหน้า)
            try:
                client.close()
                client = connect_mqtt(config)
                client.set_callback(on_message)
